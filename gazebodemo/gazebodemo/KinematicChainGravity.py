        ...
        for (joint, inertia) in zip(self.joints, self.inertias):
            ...
            if inertia is not None:
                ...
                for k in range(dof):
                    ...
                    f_g = mass * np.array([0, 0, -9.8])
                    # get current pos
                    T_curr = T @ T_from_URDF_origin(origin)
                    p_ci = p_from_T(T_curr)
                    # add torque to gravity vector
                    grav[k] += f_g @ np.cross(self.data.e[k].flatten().tolist(), (p_ci - p_from_T(self.data.T[k])).flatten().tolist())
