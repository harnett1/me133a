"""Launch Gazebo with the sevenDOF robot

This instantiates Gazebo with the "SLOW" world, i.e. running at 1/3
normal time.  It also loads the 7 DOF robot (including the associated
ROS interface) as well as a tetherball and beer can.

Finally, it also starts the classic GUI and a "tracking" node that
forces the robot in Gazebo to track the GUI commands.  This should be
a ready-to-run demo.
...
def generate_launch_description():
    ...
    #urdffile = os.path.join(pkgdir(package), 'urdf/1337dof.SLDASM.urdf')
    #with open(urdffile, 'r') as file:
    #    robot_description = file.read()

    # Locate the robot's XACRO file to convert into a URDF XML.
    # xacrofile = os.path.join(pkgdir(package), 'urdf/sevenDOF_gazebo_effort.xacro')
    # xacrofile = os.path.join(pkgdir(package), 'urdf/sevenDOF_gazebo_position.xacro')
    # xacrofile = os.path.join(pkgdir(package), 'urdf/sevenDOF_gazebo_velocity.xacro')
    xacrofile = os.path.join(pkgdir(package), 'urdf/1337dof.SLDASM.xacro')
